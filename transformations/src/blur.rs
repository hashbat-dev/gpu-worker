...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    let texel_size = vec2<f32>(1.0 / uniforms.image_width, 1.0 / uniforms.image_height);
    let blur_radius = uniforms.blur_radius;
    
    var color = vec4<f32>(0.0, 0.0, 0.0, 0.0);
    var total_weight = 0.0;
    
    // Simple box blur - can be improved with Gaussian weights
    for (var x = -blur_radius; x <= blur_radius; x += 1.0) {
        for (var y = -blur_radius; y <= blur_radius; y += 1.0) {
            ...
            if (sample_coords.x >= 0.0 && sample_coords.x <= 1.0 && 
                ...
                color += textureSample(t_diffuse, s_diffuse, sample_coords);
                total_weight += 1.0;
            }
        }
    }
