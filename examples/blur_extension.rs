...
async fn process_image(path: &str, output_path: &str, blur_radius: f32) -> Result<(), Box<dyn Error>> {
    // Load the image
    ...
    let processed_data = blur_processor.blur_image(rgba_img.as_raw(), width, height, blur_radius).await?;
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (input_path, output_path, blur_radius) = match args.len() {
        ...
        _ => {
            eprintln!("Usage: {} [input_image] [output_image] [blur_radius]", args[0]);
